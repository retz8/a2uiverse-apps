"""The Gmail app's kit config — the whole per-vendor surface, stated once."""
...
from a2uiverse_kit.paint_meta import require_carries_action
...
_AGENT_DIR = Path(__file__).resolve().parents[1]  # gmail/agent/
_APP_PKG = Path(__file__).resolve().parent  # gmail/agent/app/


def _live_toolset():
    # Deferred: app.mcp pulls google.adk + google.auth, which deterministic and stub
    # runs must never pay for.
    from app.mcp import build_gmail_toolset

    return build_gmail_toolset()
...
CONFIG = AgentAppConfig(
    ...
    app_dir=_AGENT_DIR,
    catalog_path=_AGENT_DIR.parents[0] / "gmail-catalog" / "catalogs" / "v0.9.1" / "catalog.json",
    catalog_kind="basic",
    ...
    domain_knowledge_path=_APP_PKG / "knowledge" / "gmail-domain.md",
