...
from llm_agent.mcp import GMAIL_MCP_URL, GMAIL_TOOLS, build_gmail_toolset
...
# Live Gmail by default. The stub exists so testing and manual client work need not
# touch the mailbox or consume MCP call allowance, but it is always an explicit opt-in.
DEFAULT_BACKEND = "mcp"

AGENT_NAME = "a2ui_gmail_live_agent"
...
def build_tools() -> list:
    ...
    if backend == "stub":
        logger.info(
            "tool backend: stub — canned fixture data, no Gmail calls (TOOL_BACKEND=stub)"
        )
        return list(STUB_TOOLS)
    if backend == "mcp":
        toolset = build_gmail_toolset()
        logger.info(
            "tool backend: mcp — %s, %d tools admitted (destructive tools withheld)",
            GMAIL_MCP_URL,
            len(GMAIL_TOOLS),
        )
        return [toolset]
...
def _after_tool(*, tool, args, tool_context, tool_response, **_extra):  # noqa: ANN001, ARG001
    ...
    record_shape(tool_name, args, tool_response)
