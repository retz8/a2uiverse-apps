...
Holds the app-root locator, the sibling gmail-catalog's catalog path, and the
...
_CATALOG_RELPATH = ("gmail-catalog", "catalogs", "v0.9.1", "catalog.json")


def _app_root() -> Path:
    # catalog_common/__init__.py -> catalog_common -> agent -> <app root: gmail/>
    root = Path(__file__).resolve().parents[2]
    assert (root / "gmail-catalog").is_dir(), (
        f"expected the app root containing gmail-catalog at {root}"
    )
    return root
