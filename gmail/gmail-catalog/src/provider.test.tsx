...
import {Provider, GMAIL_TOKENS, GMAIL_TOKENS_DARK} from './provider';
...
test('tokens are written on the wrapper element, never the document root', () => {
  ...
  const wrapper = container.querySelector('.gmail-catalog') as HTMLElement;
  expect(wrapper).not.toBeNull();
  for (const [token, value] of Object.entries(GMAIL_TOKENS)) {
...
test('the dark palette covers exactly the same tokens as the light one', () => {
  ...
  expect(Object.keys(GMAIL_TOKENS_DARK).sort()).toEqual(Object.keys(GMAIL_TOKENS).sort());
});
...
test('dark appearance writes the dark palette', () => {
  ...
  expect(wrapper.style.getPropertyValue('--a2ui-color-surface')).toBe(
    GMAIL_TOKENS_DARK['--a2ui-color-surface'],
  );
});
...
test('no token reads a variable the bundle does not define', () => {
  ...
  // from whichever catalog happened to set it. Gmail's product tokens are self-contained.
  for (const value of Object.values({...GMAIL_TOKENS, ...GMAIL_TOKENS_DARK})) {
