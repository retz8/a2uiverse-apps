...
type FormControlValidationViewProps = {
  text: string;
  variant: 'error' | 'success';
};

export function FormControlValidationView({text, variant}: FormControlValidationViewProps) {
  return <PrimerFormControl.Validation variant={variant}>{text}</PrimerFormControl.Validation>;
}

/**
 * Catalog entry: the generic binder resolves props, then renders FormControlValidationView.
 ...
 */
export const FormControlValidationComponent = createComponentImplementation(
  FormControlValidationApi,
  ({props}) => <FormControlValidationView text={props.text} variant={props.variant} />,
);
