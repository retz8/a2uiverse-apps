...
structural fallback having been removed -- so a question MUST be declared. There is no
structural counterpart to check it against here: the GitHub agent pairs the marker with a
ConfirmationDialog root, but the basic catalog has no rare, purpose-built dialog component
and Card roots most surfaces regardless (task-2.6 decision 15).
...
(a declined confirmation, a toggle whose result is already on screen, a request outside
the mailbox, or an action whose tool this agent does not hold). The filter strips it from
the prose and raises its `no_surface` flag; the executor accepts a surfaceless response
only under that flag -- an undeclared surfaceless response is still a validation failure
that retries.
"""
...
def validate_question_markers(payload: list[dict], metas: dict[str, dict]) -> None:
    ...
    Runs after validate_surface, feeding the same correction/retry loop. The GitHub agent
    checks the marker against a ConfirmationDialog root; that has no counterpart here,
    because the basic catalog has no rare, purpose-built dialog component and `Card` roots
    most surfaces regardless. What is checkable is semantic: a surface declared
    kind="question" must carry at least one action, or the user is asked something they
    have no way to answer. Raises ValueError naming the fix.
    """
    created: list[str] = []
