...
from llm_agent.mcp import CALENDAR_MCP_URL, CALENDAR_TOOLS, build_calendar_toolset
...
# Live Calendar by default. The stub exists so testing and manual client work need not
# touch the calendar or consume MCP call allowance, but it is always an explicit opt-in.
DEFAULT_BACKEND = "mcp"

AGENT_NAME = "a2ui_calendar_live_agent"
...
def build_tools() -> list:
    ...
    if backend == "stub":
        logger.info(
            "tool backend: stub — canned fixture data, no Calendar calls (TOOL_BACKEND=stub)"
        )
