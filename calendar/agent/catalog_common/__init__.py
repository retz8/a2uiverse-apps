...
Holds the app-root locator, the sibling calendar-catalog's catalog path, and the
...
_CATALOG_RELPATH = ("calendar-catalog", "catalogs", "v0.9.1", "catalog.json")


def _app_root() -> Path:
    # catalog_common/__init__.py -> catalog_common -> agent -> <app root: calendar/>
    root = Path(__file__).resolve().parents[2]
    assert (root / "calendar-catalog").is_dir(), (
        f"expected the app root containing calendar-catalog at {root}"
    )
    return root
