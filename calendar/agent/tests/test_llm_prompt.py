...
bulks to markers: the Calendar domain doc, the UI Description (brand doc), the catalog
schema render, and the examples. Presence assertions prove each elided bulk is actually
there. This keeps constant knowledge-doc and example edits from churning the snapshot
...
def test_skeleton_elided_all_four_bulks():
    ...
    assert "Timed and all-day events are different objects" not in skeleton
...
def test_elided_bulk_is_actually_present():
    ...
    # domain doc present (a stable heading from knowledge/calendar-domain.md)
    assert _DOMAIN_HEADING in prompt
    assert "Timed and all-day events are different objects" in prompt
