...
The agent records what it streams (llm_agent/recorder.py, armed by
A2UI_RECORD_DIR); this script supplies the prompts, threads the conversation, and
finalizes each session into a per-beat fixture under `agent/recordings/beats/`.
...
recorder — not this script — is what captures the content.
...
separate invocation against a separately-started agent:
    ...
    MODEL_NAME=gemini-3.1-pro-preview A2UI_RECORD_DIR=.recordings uv run python -m llm_agent ...
...
# The four beats (task-2.7 decision 1), one per kind of surface: list, detail, creating
# write, toggling write. Beats 2 and 3 chain onto beat 1's conversation, so the event opened
# and the event proposed both sit in a run that actually read the calendar.
#
# Re-seed the demo calendar before recording: beats 3 and 4 write to it, so a second run over
# an unseeded calendar records against drifted data (task-2.7 decision 5).
BEATS: list[Turn] = [
...
def write_fixture(spec: Turn, session: dict, turn: dict, model: str, chained_from: str | None) -> Path:
...
def drive(specs: list[Turn], model: str, record_dir: Path, url: str) -> int:
            ...
            if failure is None and session is not None:
                ...
                if all(ok for ok, _ in verdicts):
                    ...
                    for spec_in_group, turn in zip(group, recorded):
                        path = write_fixture(spec_in_group, session, turn, model, chained_from)
                        chained_from = f"beat-{spec_in_group.beat}-{spec_in_group.slug}"
                        log(f"  ok  {path.relative_to(REPO_ROOT)}")
            ...
            if attempt == MAX_ATTEMPTS_PER_BEAT:
                ...
                if session is not None:
                    ...
                    for spec_in_group, turn in zip(group, session["turns"][-len(group) :]):
                        path = write_fixture(spec_in_group, session, turn, model, chained_from)
                        chained_from = f"beat-{spec_in_group.beat}-{spec_in_group.slug}"
                        log(f"  FLAGGED (best available) {path.relative_to(REPO_ROOT)}")
...
def main() -> int:
    ...
    parser.add_argument("--beats", default="1,2,3,4,5,6,7,8", help="comma-separated beat numbers")
    ...
    return drive(specs, args.model, record_dir, args.url)
