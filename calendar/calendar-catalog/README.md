...
The Google Calendar app's A2UI catalog: the basic A2UI catalog, unchanged, in Calendar's **Material 3** look.
...
- **`Provider`**: Calendar's theme. The host wraps each of this catalog's fragments in it and sets up nothing else.

## The theme

The Provider sets its design tokens and a small stylesheet on its own wrapper element, never on the page, so the theme stays inside the fragment.
...
What the two really share stays the same: the pill button, the rounded field and chip, and the Google Sans font stack. The platform checks that the two themes really do resolve differently, with both on the page at once.
