...
surface, so the app can be driven end to end with no model call and no vendor quota. The
playing machinery — fixture load, surfaceId stamping, the visible fallback, fresh text
surfaces — is the kit's.

TODO: as the catalog grows components, replace the greeting with fixtures derived from
recorded live runs, and map each action your surfaces fire to the response it plays.
"""
...
# visible "unhandled" fallback rather than a silent no-op. Empty until the catalog holds an
