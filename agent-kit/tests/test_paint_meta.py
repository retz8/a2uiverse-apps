"""Unit tests for the paint-meta shell layer (task 8.5): tag filter + marker policies.

The filter tests are the agents' (identical ×3); the policy tests merge both agents'
suites, each against its kit-shipped named policy.
"""
...
# --- require_carries_action (the basic-catalog agents' policy) ---


def _actionable_payload(surface_id: str, *, answerable: bool) -> list[dict]:
...
def test_a_declared_question_carrying_an_action_passes():
    ...
    require_carries_action(_actionable_payload("q1", answerable=True), metas)
...
def test_a_declared_question_with_no_action_is_rejected():
    ...
    with pytest.raises(ValueError, match="carries no action"):
        require_carries_action(_actionable_payload("q1", answerable=False), metas)


def test_an_ordinary_surface_needs_no_action():
    metas = {"s1": {"surfaceId": "s1", "title": "Waiting on you"}}
    require_carries_action(_actionable_payload("s1", answerable=False), metas)
...
def test_an_undeclared_surface_is_never_forced_to_be_a_question():
    ...
    require_carries_action(_actionable_payload("s1", answerable=True), {})


# --- require_root_component (the custom-catalog agent's policy) ---

_dialog_policy = require_root_component("ConfirmationDialog")


def _rooted_payload(surface_id: str, root_component: str) -> list[dict]:
    return [
        {"version": "v0.9", "createSurface": {"surfaceId": surface_id, "catalogId": "c"}},
        {
            "version": "v0.9",
...
def test_marker_with_dialog_root_passes():
    metas = {"q1": {"surfaceId": "q1", "kind": "question"}}
    _dialog_policy(_rooted_payload("q1", "ConfirmationDialog"), metas)
...
def test_unmarked_non_dialog_surface_passes():
    _dialog_policy(_rooted_payload("s1", "PageLayout"), {})
