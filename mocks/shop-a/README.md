...
An A2A agent that answers questions about Aperture & Co's cameras with UI it generates in A2UI.

Aperture & Co is invented: a mock store, a boutique camera dealer, with no real backend.

- **Dataset**: [`mocks/dataset/products.json`](../dataset/products.json), in place of an MCP server. [Shop B](../shop-b/) reads the same file; the two stores list the same cameras, each with its own prices, ratings and stock.
...
- **Catalog** ([`shop-a-catalog/`](shop-a-catalog/)): A2UI components that UI is built from, in a neutral theme with a warm-brown accent. A2UI is a protocol for agents to generate UI.
