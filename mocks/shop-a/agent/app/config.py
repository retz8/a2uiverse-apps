"""The Shop A app's kit config — the whole per-vendor surface, stated once.
...
_AGENT_DIR = Path(__file__).resolve().parents[1]  # shop-a/agent/
_APP_PKG = Path(__file__).resolve().parent  # shop-a/agent/app/
...
CONFIG = AgentAppConfig(
    ...
    skills=SKILLS,
    default_port=12001,
    responder_app_name="a2ui_shop_a_live",
    adk_agent_name="a2ui_shop_a_live_agent",
    app_dir=_AGENT_DIR,
    catalog_path=_AGENT_DIR.parent / "shop-a-catalog" / "catalogs" / "v0.9.1" / "catalog.json",
    catalog_kind="basic",
    ...
    domain_knowledge_path=_APP_PKG / "knowledge" / "shop-a-domain.md",
