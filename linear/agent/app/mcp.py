...
stub corpus. Nothing is pseudonymized: the data is the user's own workspace, as a Linear user
sees it.
"""
...
class RecordingMcpTool(PolicyMcpTool):
    ...
    A comment list names no issue, so its capture carries the `issueId` it was read for — the
    key the stub serves it back under.
    """
...
def build_live_toolset() -> RecordingMcpToolset:
    ...
    but the key is read here, so a missing one fails at startup.
    """
