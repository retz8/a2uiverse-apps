...
def test_catalog_path_points_at_sibling_package():
    ...
    assert path.is_file()
    assert path.parts[-4:] == ("linear-catalog", "catalogs", "v0.9.1", "catalog.json")


def test_catalog_id_is_the_hosted_catalog_url():
    cid = _ctx.get_catalog().catalog_id
    assert cid == "https://github.com/retz8/a2uiverse-apps/blob/main/linear/linear-catalog/catalogs/v0.9.1/catalog.json"
