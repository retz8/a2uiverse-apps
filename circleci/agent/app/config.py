"""The CircleCI app's kit config — the whole per-vendor surface, stated once.
...
_AGENT_DIR = Path(__file__).resolve().parents[1]  # circleci/agent/
_APP_PKG = Path(__file__).resolve().parent  # circleci/agent/app/
...
CONFIG = AgentAppConfig(
    ...
    skills=SKILLS,
    default_port=11004,
    responder_app_name="a2ui_circleci_live",
    adk_agent_name="a2ui_circleci_live_agent",
    app_dir=_AGENT_DIR,
    catalog_path=_AGENT_DIR.parent / "circleci-catalog" / "catalogs" / "v0.9.1" / "catalog.json",
    catalog_kind="basic",
    ...
    domain_knowledge_path=_APP_PKG / "knowledge" / "circleci-domain.md",
